...
pub struct MessageQueue {
    ...
    index_map: HashMap<ChannelId<Messages>, IdxVec>,
}
...
impl MessageQueue {
    ...
    pub fn reserve_channel(&mut self, channel: ChannelId<Messages>, additional: usize) {
...
pub struct ChannelIter<'a> {
    messages: &'a [Message],
    map_iter: bevy::utils::hashbrown::hash_map::Iter<'a, ChannelId<Messages>, IdxVec>,
}

impl<'a> Iterator for ChannelIter<'a> {
    type Item = (ChannelId<Messages>, MessageIter<'a>);
...
fn message_queue_ordering_test() {
    ...
    queue.push_channel(ChannelId::<Messages>::from(0), map_messages(MESSAGE_SET_A));
    queue.push_channel(ChannelId::<Messages>::from(0), map_messages(MESSAGE_SET_B));
    queue.push_channel(ChannelId::<Messages>::from(1), map_messages(MESSAGE_SET_C));

    queue.iter_channel(ChannelId::<Messages>::from(0))
    .zip(MESSAGE_SET_A.iter().chain(MESSAGE_SET_B))
    .for_each(|(a, b)| assert_eq!(a.as_slice(), *b));

    queue.iter_channel(ChannelId::<Messages>::from(1))
    .zip(MESSAGE_SET_C)
    .for_each(|(a, b)| assert_eq!(a.as_slice(), *b));
}